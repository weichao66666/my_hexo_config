...
1. 按返回键会先判断后退栈中是否有元素，因为有"web_wrapper"，然后Java会调用JS的clickBack("web_wrapper")，clickBack("web_wrapper")对应于html中按钮的点击事件，即会弹出对话框，同时JS会调用Java的removeBack("web_wrapper")，删除后退栈中的元素"web_wrapper"。
1. 再次点击返回键会直接触发对话框的取消，同时JS会调用Java的back("web_wrapper")，则此时后退栈添加元素"web_wrapper"，又回到最初加载时的状态。
